    ...
    def _get_model(self):
        if self._model is None:
            try:
                ...
                self._model = UniMERNet.from_pretrained(
                    "佑思科技/UniMERNet", device=self.device
                )
            except ImportError:
                raise ImportError(
                    "UniMERNet not installed. Install with: pip install unimernet"
                )
        return self._model
